...
def building_block(inputs, double_filters, weight_decay, dropout, include_batch_norm):
...
def build_wrn(inputs, n_classes, first_layer_kernel=(3, 3), first_layer_strides=(1, 1), groups=3, blocks_in_groups=1, filters_mult=1, dropout=0., weight_decay=0., include_softmax=True, include_batch_norm=True):
    """Create WRN."""
    n_filters_1st_layer = 16 * filters_mult
    t = inputs
    if include_batch_norm:
        t = layers.BatchNormalization(name='bn0')(t)
